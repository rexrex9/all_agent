...
def create_or_clear_presentation(filename: str) -> str:
    ...
    return f"Presentation '{filename}' created/cleared successfully."
...
async def get_slide_content_description(filename: str, slide_index: str) -> str:
    ...
    for i, shape in enumerate(slide.shapes):
        ...
        if shape.has_text_frame and shape.text.strip():
            ...
            text_preview_replaced=text_preview.replace(chr(11), ' ').replace('\n', ' ')
            desc += f", Text='{text_preview_replaced}'" # Replace VT and newlines for single line desc
