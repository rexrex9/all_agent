...
def get_out_path(file_path):
    if type(file_path) != str:
        return file_path
    dir = os.path.normpath(ROOT_DIR)
    file_path = os.path.normpath(file_path)
    relative_path = file_path.replace(dir, '')
...
def change_file_path(file_path):
    ...
    return os.path.normpath(os.path.join(dir, file_path))

# 将命令中的文件路径转换为绝对路径
def change_command_path(command):
    paths = find_path(command)
    for path in paths:
        changed_path = change_file_path(path)
...
class ExecuteMiddleware(AgentMiddleware):
    ...
    def wrap_tool_call(self, request, handler):
        ...
        if isinstance(tool_result, ToolMessage):
            # 只转换内容中的路径，其他信息保持不变
            tool_result.content = get_out_path(tool_result.content)
